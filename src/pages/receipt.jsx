...
import { useParams } from "react-router-dom"; // import for reading URL id
import "./receipt.css";

const API_URL = "https://backup-0k8h.onrender.com";

function Receipt() {
  const { id } = useParams(); // get selected id from URL
  ...
  if (!receipt) return <p>לא נמצאה קבלה עם מזהה {id}</p>;

  const amount = Number(receipt.amount || 0) || 0;
